  ...
  axios
    ...
    .then(response => {
      if (response.status === 200) {
        ...
        const $ = cheerio.load(html);
        let temp = $(
          '._-_-node_modules--wxu-components-src-organism-CurrentConditions-CurrentConditions--tempValue--3KcTQ'
        ).text();
        let desc = $(
          '._-_-node_modules--wxu-components-src-organism-CurrentConditions-CurrentConditions--phraseValue--2xXSr'
        ).text();
        let feels = $(
          '._-_-node_modules--wxu-components-src-organism-TodayDetailsCard-TodayDetailsCard--feelsLikeTempValue--2aogo'
        ).text();
        let location = $(
          '._-_-node_modules--wxu-components-src-organism-CurrentConditions-CurrentConditions--location--1Ayv3'
        ).text();
        let prec = $(
          '._-_-node_modules--wxu-components-src-organism-CurrentConditions-CurrentConditions--precipValue--RBVJT'
        ).text();
